% Test de convergencia k (TIEMPO) - PARTE 2 (u_t = u_xx + f(x,t) y condición de borde)
% Usar T = 0.5, T = 5 o 10, tarda demasiado y consume demasiados recursos
% dando los mismos resultados

close all;
clear;
disp(' INICIANDO TEST CONVERGENCIA K (Parte 2) ');
...
for i = 2:3 
    ...
    fprintf("P2 (k): Ejecutando método %s...\n", method_names{i});
    ...
    % Leyenda
    title(sprintf('P2 Convergencia Temporal (q) - %s', method_names{i}));
    xlabel('log(k)'); 
    ylabel('log(Error)');
    legend('Datos', sprintf('Ajuste (q = %.4f)', orden_q), 'Location', 'best');
    grid on;

    print(sprintf('P2_convergencia_k_Metodo_%d.png', i), '-dpng');
end
