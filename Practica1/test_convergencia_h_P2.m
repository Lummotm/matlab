% Test de convergencia h (ESPACIO) - PARTE 2 (u_t = u_xx + f(x,t))

close all;
clear;
disp(' INICIANDO TEST CONVERGENCIA H (Parte 2) ');


T = 0.5; % Se emplea T = 0.5 para optimizar el coste computacional, sin alterar el análisis.
...
for i = 1:3 
    ...
    fprintf("P2 (h): Ejecutando método %s...\n", method_names{i});
    
    [~, Errors, ~, ~, ~] = practica1_2(choiceMethod, J_values, N_values, T);
    ...
    % Leyenda
    title(sprintf('P2 Convergencia Espacial (p) - %s', method_names{i}));
    xlabel('log(h)');
    ylabel('log(Error)');
    ...
    print(sprintf('P2_convergencia_h_Metodo_%d.png', i), '-dpng');
end

%  Prueba Adicional: Implícito Sin Ajuste (P2) 
...
% Leyenda
title(sprintf('P2 Convergencia Espacial Sin Ajuste (p) - %s', method_names{i}));
xlabel('log(h)');
ylabel('log(Error)');
legend('Datos', sprintf('Ajuste (p = %.4f)', orden_p), 'Location', 'best');
grid on;
...
% Denotamos en función del método via sprintf
print(sprintf('P2_convergencia_h_Metodo_%d_no_ajustado.png', i), '-dpng');
